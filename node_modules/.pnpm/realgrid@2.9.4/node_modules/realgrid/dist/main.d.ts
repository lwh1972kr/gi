...
export declare interface CopyOptions {
    ...
    /**
     * 다중 선택 후 복사할때 발생하는 error 메시지
     */
    multiSelectionCopyFailText?: string;
}
...
export declare abstract class DataProviderBase {
    ...
    /**
     * {@link DataOptions.softDeleting}이 `true`인 경우, 상태가 {@link RowState.DELETED}로 표시된 행을 원래 상태로 복원한다.
     *
     * @param rows - 원복하려는 데이터행 인덱스의 배열
     *
     * @example
     * ```js
     * ds.revertDeletedStates([0, 1]);
     * ```
     */
    revertDeletedStates(rows: number | number[]): void;
    /**
     * 지정한 행들의 값과 상태를 복원한다.
     * @remarks
...
export declare interface DataProviderConfig extends DataOptions {
    /**
     * 필드를 설정한다
     */
    fields?: (string | DataFieldObject)[];
}
 ...
 * ```
 */
export declare type DisplayCallback = (grid: GridBase, index: CellIndex, value: any) => any;
...
export declare interface DisplayOptions {
    ...
    rowHeight?: number;
    /**
     * 자동 행 높이(rowHeight = -1) 사용 시,
     * 계산된 높이에 여유 공간으로 추가되는 픽셀 수.
     *
     * @remarks
     * 자동 행 높이가 활성화된 경우에만 적용된다.
     * 기본값은 0이며, 지정한 값만큼 행 높이가 더 커진다.
     *
     * @defaultValue 0
     */
    rowHeightPadding?: number;
    /**
     * Table 행 최소 높이
     *
     * @defaultValue `23`
     ...
     *
     * @remarks
     * `always`로 설정하면 스크롤가능여부에 상관없이 항상 스크롤바가 보인다.
     *
     * @defaultValue `true`
     */
    hscrollBar?: boolean | "always";
    /**
     * 수직 스크롤 바 표시 여부
     *
     * @remarks
...
export declare type EventHandlers<T> = Pick<T, Extract<keyof T, `on${string}`>>;
 ...
 * [반환값] - 내보내기 될 값
 */
export declare type ExportValueCallback = (index: number, name: string, value: any, grid: GridBase | DataProviderBase, cellIndex?: CellIndex) => any;
...
export declare abstract class GridBase {
     ...
     * @example
     * ```js
     * gridView.onCellPasting = function (grid, index, value, editResult) {
     *      if (value === 'hello') {
     *          return false;
     *      }
     *      if (editResult.oldValue === 'expired') {
     *          return false;
     *      }
     *      return true;
     * };
     ...
     *
     * `isEllipsis` - 데이터 셀이 ellipsis상태인지 여부
     *
     ...
     * ```
     */
    onShowTooltip?: (grid: GridBase, index: CellIndex, value: any, isEllipsis: boolean) => boolean | string;
    /**
     ...
     * ```
     */
    onShowHeaderTooltip?: (grid: GridBase, column: GridColumn, value: any, layout: CellLayoutItem, isEllipsis: boolean) => string;
    /**
     *
     * @deprecated
     ...
     * @example
     * ```js
     * gridView.onInitialized = function (grid) {
     *      console.log(`gridView initialized`);
     * }
     * ```
     */
    onInitialized?: (grid: GridBase) => void;
    /**
    ...
    /**
     * header의 높이를 글자 높이에 맞게 변경한다.
     *
     * @remarks
     * 표시된 글자가 모두 표시되도록 높이를 변경한다
     *
     * @param apply - 높이 변경 여부
     * @param paddingHeight - 추가되는 여백
     *
     * @example
     * ```js
     *  gridView.fitHeaderHeight(true, 2);
     * ```
     */
    fitHeaderHeight(apply?: boolean, paddingHeight?: number): void;
    /**
     * 행 높이를 지정한 높이로 변경한다.
     *
     *
...
export declare interface GridBaseConfig extends GridOptions, EventHandlers<GridBase> {
    /**
     * 그리드에 표시되는 컬럼 정보
     *
     * @remarks
     * {@link GridBase.setColumns} 참조
     */
    columns?: (DataColumn | string | LiteralColumn)[];
    /**
     * 그리드의 컬럼 레이아웃을 설정.
     *
     * @remarks
     * {@link GridBase.setColumnLayout} 참조
     */
    layout?: (LayoutItem | string)[];
    /**
     * 그리드에서 오른쪽 마우스를 클릭했을때 표시되는 컨텍스트 메뉴에 추가할 메뉴 항목들을 지정한다.
     * @remarks
     * {@link GridBase.setContextMenu} 참조
     */
    contextMenu?: (PopupMenu | PopupMenuItem)[];
    /**
     * rowStyleCallback을 설정.
     *
     * @remarks
     * {@link GridBase.setRowStyleCallback} 참조
     */
    rowStyleCallback?: RowStyleCallback;
    /**
     * cellStyleCallback을 설정.
     *
     * @remarks
     * {@link GridBase.setCellStyleCallback} 참조
     */
...
export declare interface GridHeader {
     ...
     *
     * 3. `0` 미만: 브라우저가 계산한 높이 //deprecated
     *
...
export declare interface GridOptions {
    ...
    /**
     * DataProvider의 softDeleting이 `true`일때 삭제된 행들의 숨김 여부
     */
     ...
     *
     * @remarks
     * `true`이면 commit되기 전까지 편집된 내용을 undo/redo 할수 있다.
     */
    undoable?: boolean;
}
...
/**
 * 그리드 설정정보
 * {@link RealGrid.createGrid}를 이용해서 그리드를 생성할때 사용된다.
 */
export declare interface GridViewConfig extends ViewOptions, GridBaseConfig, EventHandlers<GridView> {
    /**
     * 웹접근성 모드로 생성하는 경우 `true`로 설정한다.
     */
    accessibility?: boolean;
    /**
     * 웹접근성 모드로 생성되었을때 HTMLTableElement의 caption을 설정하기 위한 설정
     */
    waiOptions?: WaiOptions;
    /**
     * 행 그룹 모델
     */
    rowGroup?: RowGroup;
...
export declare interface LocalDataProviderConfig extends DataProviderConfig, EventHandlers<LocalDataProvider> {
    /**
     * 데이터 셋에서 데이터를 필터링한다.
     *
     * @remarks
     * {@link LocalDataProvider.setFilters} 참조
     */
     ...
     *
     * @remarks
     * {@link LocalDataProvider.setFilterMode} 참조
     */
    filterMode?: "and" | "or";
}
...
export declare interface LocalTreeDataProviderConfig extends DataProviderConfig, EventHandlers<LocalTreeDataProvider> {
}
 ...
 *
 */
export declare type MergeCallback = (grid: GridBase, column: DataColumn, itemIndex: number, value: any) => any;
 ...
 *
 */
export declare type ObjectToValuesCallback = (ds: DataProviderBase, obj: ConfigObject) => DataValues;
...
export declare interface PasteOptions {
    ...
    forceAppend?: boolean;
    /**
     * pasteOptions의 selectBlockPaste가 true일때, 다중 선택 후 불여넣기할때 발생하는 error 메시지
     */
    multiSelectionPasteFailText?: string;
}
...
declare class RealGrid {
    ...
    /**
     * container와 설정정보를 입력해서 그리드를 생성한다.
     *
     * @param container - 그리드가 위치할 div의 id또는 div 객체
     * @param viewConfig - 그리드의 설정값
     * @param dataConfig - LocalDataProvider의 설정값
     * @returns - 그리드와 dataProvider의 instance
     */
    static createGrid(container: string | HTMLDivElement, viewConfig?: GridViewConfig, dataConfig?: LocalDataProviderConfig): {
        gridView: GridView;
        dataProvider: LocalDataProvider;
    };
    /**
     * container와 설정정보를 입력해서 트리그리드를 생성한다.
     *
     * @param container - 트리그리드가 위치할 div의 id또는 div 객체
     * @param viewConfig - 트리그리드의 설정값
     * @param dataConfig - LocalTreeDataProvider의 설정값
     * @returns - 트리그리드와 dataProvider의 instance
     */
    static createTree(container: string | HTMLDivElement, viewConfig: TreeViewConfig, dataConfig?: LocalTreeDataProviderConfig): {
        treeView: TreeView;
        dataProvider: LocalTreeDataProvider;
    };
}
...
export declare class TreeView extends GridBase {
    ...
    setOptions(options: TreeViewOptions): void;


}

export declare interface TreeViewConfig extends GridOptions, GridBaseConfig, EventHandlers<TreeView> {
    /**
     * TreeView 설정 모델
     */
 ...
 * {@link GridOptions}
 */
export declare interface TreeViewOptions extends GridOptions {
    treeOptions?: TreeOptions;
}
